<div class="col-xs-12 col-md-12 col-lg-9 col-xl-9">
  <?php if (!$pager->getNbResults()): ?>
    ...
    <a href="<?php echo url_for('noticias_new') ?>" class="btn btn-primary">Agregar nueva noticia</a>
  <?php else: ?>
    ...
    <div class="table-responsive">
      <table class="table table-hover table-striped">
        <thead>
          <tr>
            ...
            <th class="text-center">
              <a class="link_actions" title="Agregar" href="<?php echo url_for('@noticias_new') ?>">
        ...
        <tbody>
          <?php foreach ($pager->getResults() as $i => $noticias): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
            <tr class="sf_admin_row <?php echo $odd ?>">
              <?php include_partial('news/list_td_batch_actions', array('noticias' => $noticias, 'helper' => $helper)) ?>
              <?php include_partial('news/list_td_tabular', array('noticias' => $noticias)) ?>
              <?php include_partial('news/list_td_actions', array('noticias' => $noticias, 'helper' => $helper)) ?>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
